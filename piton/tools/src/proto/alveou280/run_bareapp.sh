...
sleep 2
dma-ctl qdma08000 reg write bar 2 0x0 0x1D #Release system reset, we must wait until the memory is filled with 0s
sleep 2
#pitonstream must be running and at this moment, it says "TEST_OUTPUT", and it is waiting for Arine reset to be released 
dma-ctl qdma08000 reg write bar 2 0x0 0x1F #Release Ariane's reset
