...
Moreover, there are two scripts that run all physical and virtual tests. They just check if simulation finalized correctly. To run any of them:

    #  set environment if not
    source /home/tools/riscv_vector_toolchain/set_env.sh
    source /apps/env.sh
    cd build
    ./run_all_isa_test.sh     # for physical tests
    ./run_all_isa_v_test.sh   # for virtual tests

Results can be found under: isa_test_result.csv or isa_v_test_result.csv
...
PMU monitor and scoreboard is used to compare hardware performance counters from CSR file against the reference counters from the pmu_mon module. To enable this check, -pmu_mon flag can be used with the sims command.

```
    sims -sys=manycore -x_tiles=1 -y_tiles=1 -msm_build -lagarto -config_rtl=BSC_RTL_SRAMS -config_rtl=OPENPITON_LAGARTO_COMMIT_LOG -config_rtl=FPU_ZAGREB -pmu_mon
    sims -sys=manycore -msm_run -x_tiles=1 -y_tiles=1 rv64ui-p-addi.S -lagarto -precompiled -pmu_mon
```

#### Running manycore 

Currently in order to run some tests, ariane's repo has to be download. Apart from that, specific flags are required (finish_mask & rtl_timeout). Finish_mask ensures that all cores hit a good (or any hits a bad trap) to finalize the simulation. The number of '1' following -finish_mask 0x, should match the number of instantiated cores.

```
# download ariane's repo
git submodule update --init --recursive piton/design/chip/tile/ariane
# build project
sims -sys=manycore -x_tiles=3 -y_tiles=2 -msm_build -lagarto -config_rtl=BSC_RTL_SRAMS -config_rtl=OPENPITON_LAGARTO_COMMIT_LOG
# run simulation
